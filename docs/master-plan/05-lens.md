...
| 2026-07-18 | Recurrence joined the shared contract (`recurrence`/`recurrence_until`/`recurrence_days_of_week`) across ALL THREE readers | Posters/pages/prompts state "çdo të premte" and "22–24 gusht" all the time and the wizard has carried recurrence since Phase 15 — dropping it forced manual re-entry. Consecutive-day ranges map to daily+until (replacing the old "first day, mention range in description" rule); stated-only, never guessed |
| 2026-07-18 | Weekly with no weekday list derives the day from the resolved date; until dropped when before the date or >366 days out | The resolved date IS an occurrence, so its weekday is derivation not invention; the span caps turn range misreads into safe one-offs instead of calendar spam |
