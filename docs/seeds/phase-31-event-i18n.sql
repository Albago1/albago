-- =============================================================================
-- Phase 31 — LENS-3 auto-translation storage (event title/description i18n)
-- =============================================================================
-- Adds `title_i18n` and `description_i18n` (jsonb, keyed by language code:
-- en/sq/de/es) to events + event_submissions. Populated only for events
-- created via the Lens poster scanner; null for everything else. The public
-- event page renders the viewer's language and falls back to the original
-- title/description when a translation is missing.
--
-- The submit_event_submission RPC is re-created to carry the two packs from
-- the payload (full body otherwise identical to phase-27, which added
-- address_hint). Presence of a pack IS the "this was scanned" marker — typed
-- submissions send null and every path no-ops.
--
-- Additive + idempotent — safe to re-run. Reversing = drop the columns and
-- re-run the phase-27 function body.
-- =============================================================================
...
alter table public.event_submissions add column if not exists title_i18n jsonb;
alter table public.event_submissions add column if not exists description_i18n jsonb;
...
begin
  ...
  insert into public.event_submissions (
    title, title_i18n, venue_name, place_id, date, time, end_time, timezone,
