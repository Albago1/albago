  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div>
          <SectionLabel>{t('safety_label')}</SectionLabel>
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2">
        <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.04] p-5">
          <p className="text-sm font-semibold text-emerald-300">{t('safety_we_support')}</p>
        ...
        <div className="rounded-2xl border border-flame-500/20 bg-flame-500/[0.04] p-5">
          <p className="text-sm font-semibold text-flame-300">{t('safety_we_do_not')}</p>
          <ul className="mt-3 space-y-2 text-sm leading-6 text-white/75">
            {NOT_SUPPORTED_KEYS.map((key) => (
              <li key={key}>• {t(key)}</li>
            ))}
          </ul>
        </div>
      </div>
      <p className="mt-6 text-xs text-white/45">{t('safety_footnote')}</p>
    </div>
  )
}
