  ...
  if (placards.length === 0) {
    return (
      <div className="mt-4 rounded-3xl border border-dashed border-white/10 bg-white/[0.02] p-8 text-center">
        ...
        <p className="mt-4 font-semibold text-white">No placards yet</p>
        <p className="mt-1 text-sm text-white/50">
          Upload a photo of your sign at a protest and it will appear here.
        </p>
        <Link
          href="/pankartat"
            ...
            <div className="flex min-w-0 flex-1 flex-col justify-between">
              <div>
                ...
                {p.caption ? (
                  ...
                  <p className="mt-2 text-sm italic text-white/40">No caption</p>
                )}
              </div>
              ...
              {p.status === 'approved' && (
                <div className="mt-2 flex items-center gap-3 text-[11px] text-white/55">
                  <span>{p.voteCount} likes</span>
                  {p.reportCount > 0 && (
                    <span className="text-flame-300">{p.reportCount} reports</span>
                  )}
                </div>
              )}
            </div>
